...
import (
	...
	"seckill-system/internal/activity/model"
...
func (s *ActivityService) CreateActivity(req *activity.CreateActivityReq) (*activity.CreateActivityResp, error) {
	if req.Stock <= 0 {
		return &activity.CreateActivityResp{Code: errno.ErrParam.Code, Msg: "库存必须大于0"}, nil
	}

	startTime, err := time.Parse(time.RFC3339, req.StartTime)
	if err != nil {
		return &activity.CreateActivityResp{Code: errno.ErrParam.Code, Msg: "开始时间格式错误"}, nil
	}
	endTime, err := time.Parse(time.RFC3339, req.EndTime)
	if err != nil {
		return &activity.CreateActivityResp{Code: errno.ErrParam.Code, Msg: "结束时间格式错误"}, nil
	}
	if endTime.Before(startTime) {
		return &activity.CreateActivityResp{Code: errno.ErrParam.Code, Msg: "结束时间必须晚于开始时间"}, nil
	}
...
func (s *ActivityService) GetActivity(req *activity.GetActivityReq) (*activity.GetActivityResp, error) {
	...
	if err := dal.DB.First(&act, req.ActivityId).Error; err != nil {
		return &activity.GetActivityResp{Code: errno.ErrNotFound.Code}, nil
	}
